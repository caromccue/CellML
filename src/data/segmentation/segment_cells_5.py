...
def segment5(img, postsize=100, exp_clip_limit=15):
    '''
...
def extract_indiv_cells5(img, labeled, border=15, area_upper_cutoff=2.5, area_lower_cutoff=1.25, ecc_cutoff=0.5, ecc_cutoff_upper=1):
    '''
    ...
    for region in reg_clean:
        ...
        img_list5.append(contrast_stretch)
        area_list5.append(region.area)

    return img_list5, reg_clean, area_list5

def segment_cells_to_file5(image_filename, save_overlay=False):
    ...
    for image_file in tqdm(img_list):
        ...
        # Segment image
        (labeled, num_regions) = segment5(image)
        ...
        cell_images, _, area_list5 = extract_indiv_cells5(image, labeled)
        ...
        for (i, img) in enumerate(cell_images):
            name5 = os.path.join(out_directory, os.path.basename(image_file).split('.')[0] + '_cell_' + str(i) + '.jpg')
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
