...
def segmentflat(img, postsize=50, exp_clip_limit=4):
    ...
    ----------
    img: numpy.ndarray
        Array representing the greyscale values (0-255) of an image cropped to show only the cell region
...
def extract_indiv_cellsflat(img, labeled, border=15, area_upper_cutoff=3, area_lower_cutoff=0.33, ecc_cutoff=0.05, ecc_cutoff_upper=1):
    ...
    for region in reg_clean:
        ...
        img_listflat.append(contrast_stretch)
        area_listflat.append(region.area)

    return img_listflat, reg_clean, area_listflat

def segment_cells_to_fileflat(image_filename, save_overlay=False):
    ...
    for image_file in tqdm(img_list):
        ...
        # Segment image
        (labeled, num_regions) = segmentflat(image)
        ...
        cell_images, _, area_listflat = extract_indiv_cellsflat(image, labeled)
