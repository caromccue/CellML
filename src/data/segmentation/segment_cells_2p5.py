...
def extract_indiv_cells2p5(img, labeled, border=15, area_upper_cutoff=3, area_lower_cutoff=2, ecc_cutoff=0.5, ecc_cutoff_upper=0.95):
    '''
...
def segment_cells_to_file2p5(image_filename, save_overlay=False):
    ...
    for image_file in tqdm(img_list):
        ...
        cellarea2p5 = {name2p5[j] : area_list2p5[j] for j in range(len(name2p5))} 
