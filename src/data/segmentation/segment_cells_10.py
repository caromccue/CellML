...
def segment10(img, postsize=275, exp_clip_limit=10):
    '''
...
def extract_indiv_cells10(img, labeled, border=15, area_upper_cutoff=3, area_lower_cutoff=2, ecc_cutoff=0.25, ecc_cutoff_upper=1):
    ...
    for region in reg_clean:
        ...
        img_list10.append(contrast_stretch)
        area_list10.append(region.area)

    return img_list10, reg_clean, area_list10

def segment_cells_to_file10(image_filename, save_overlay=False):
    ...
    for image_file in tqdm(img_list):
        ...
        cellarea10 = {name10[k] : area_list10[k] for k in range(len(name10))} 
